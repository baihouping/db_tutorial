...
void* get_page(Pager* pager, uint32_t page_num) {
  ...
  if (pager->pages[page_num] == NULL) {
    ...
    void* page = malloc(PAGE_SIZE_DB);
    uint32_t num_pages = pager->file_length / PAGE_SIZE_DB;
    // We might save a partial page at the end of the file
    if (pager->file_length % PAGE_SIZE_DB) {
      num_pages += 1;
    }
    if (page_num <= num_pages) {
      lseek(pager->file_descriptor, page_num * PAGE_SIZE_DB, SEEK_SET);
      ssize_t bytes_read = read(pager->file_descriptor, page, PAGE_SIZE_DB);
      if (bytes_read == -1) {
...
void pager_flush(Pager* pager, uint32_t page_num, uint32_t size) {
  ...
  off_t offset = lseek(pager->file_descriptor, page_num * PAGE_SIZE_DB, SEEK_SET);
...
void db_close(Table* table) {
  ...
  for (uint32_t i = 0; i < num_full_pages; i++) {
    ...
    pager_flush(pager, i, PAGE_SIZE_DB);
    free(pager->pages[i]);
    pager->pages[i] = NULL;
  }
